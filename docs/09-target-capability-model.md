...
- `GraphMemoryProductionRuntimeReport` pass requires row 045 live normalization, row 047 live evidence, row 050 multi-agent repair, advanced graph, graph frontier/review, temporal KG, and memory kernel refs plus URL, redirect, canonical, page-structure, entity, task, temporal graph refs, site/task/repair/run-diary memory refs, freshness, invalidation, frontier and repair explanations, source evidence, verification, policy, command/event/outbox, and replay refs
- graph and memory can improve planning, frontier priority, repair, and operator explanation, but graph, temporal KG, memory, and agent reasoning refs cannot satisfy source evidence, verification, publication, or output manifest requirements
- missing graph/memory dependencies, graph-as-evidence, memory-as-evidence, stale memory use, missing invalidation, missing explanations, or replay mismatch fail deterministically
