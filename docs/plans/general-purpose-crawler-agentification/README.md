...
| s2 | LLM-driven `CrawlPlanner` adapter via `ModelProviderPortV2` | 1 | Production adapter consuming s1 port; writes prompt-template ref, model-call-trace ref, and seed_ref into `PlanDecision.replay_refs`; gated behind PRODUCTION mode. |
| s3 | `ExternalCrawlRunner` wires `CrawlPlannerPort` | 1 | Runner invokes the planner before seed enqueue; consumes `frontier_priority_hints` and `adapter_priors`; persists `PlanDecision` ref into the run report. Live integration test against `httpbin.org` + one new fixture site. |
...
| s5 | `PlannerObservationFeedback` contract + deterministic fixture planner adapter v2 | 2 | Planner accepts a `GraphSnapshotRef` input; deterministic adapter refines `frontier_priority_hints` based on observed canonical / redirect / structural neighbours. |
| s6 | Runner persists graph events during real runs and re-invokes planner on frontier-empty | 2 | Live wiring of s4 + s5; graph snapshot ref attached to each planner invocation; replay re-runs deterministically given the snapshot. |
