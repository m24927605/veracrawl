...
| s6 | Runner persists graph events during real runs and re-invokes planner on frontier-empty | 2 | Live wiring of s4 + s5: the **caller** wires `InMemoryGraphObserver` (s4) and `lambda fb: DeterministicCrawlPlannerV2(feedback=fb)` (s5) at the composition root and passes them to the runner via new ctor args (`graph_observer: GraphObservationPort`, `feedback_aware_planner_factory: Callable[[PlannerObservationFeedback], CrawlPlannerPort]`, plus a `utc_clock` for graph-event UTC datetimes). Runner records events through the port as fetches/redirects/parses happen, builds a snapshot at the first frontier-empty after ≥ 1 fetch, derives a `PlannerObservationFeedback`, threads `feedback.id` into `PlanRequest.observed_state_refs`, and re-invokes the planner once via the factory. Replay re-runs deterministically given the same `CrawlJobSpec` + `utc_clock` + fetcher (`feedback.id` participates in `decision.replay_refs`). The runner does NOT import any `veracrawl.adapters.*` — enforced by an AST-walk import-boundary test. |
