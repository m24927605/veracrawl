...
| s6 | `ExternalCrawlRunner` wires `GraphObservationPort` + replan via feedback-aware planner | IMPL_IN_PROGRESS | f986701 | step 1: 36dd567 → e3ea052 → 5d8c6ed → 5a0bdb2 (all REJECTED iter 1-4) → 944a15f (REJECTED iter 5: STATUS summary still out of sync — cap reached) → this commit (DONE_WITH_RESERVATIONS; STATUS fix landed inline, no re-review). step 2+: pending. | iter 1 REJECTED, iter 2 REJECTED, iter 3 REJECTED, iter 4 REJECTED, iter 5 REJECTED (5/5 used; v6 follow-up landed without re-review) — see below | step 1: iter 1→5 REJECTED → DONE_WITH_RESERVATIONS (5/5 used). | 4 — see "s6 plan iter-5 reservations" below |
