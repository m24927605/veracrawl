...
| s2 | Real LLM-driven `CrawlPlanner` adapter (replay-strict) | IMPL_IN_PROGRESS | d271e04 | step 1: 9ab4de3, 8cca1b5 (contracts + 14 tests). | iter 1 REJECTED, iter 2 REJECTED, iter 3 REJECTED, iter 4 REJECTED, iter 5 REJECTED (5/5 used; post-iter-5 follow-up landed without re-review) — see below | step 1: iter 1 REJECTED → iter 2 in flight — see below | 1 — see "s2 plan iter-5 reservations" below |
