...
| s6 | `ExternalCrawlRunner` wires `GraphObservationPort` + replan via feedback-aware planner | IMPL_IN_PROGRESS | f986701 | step 1: 36dd567 → e3ea052 → 5d8c6ed → 5a0bdb2 → 944a15f (all REJECTED iter 1-5) → f749385 (DONE_WITH_RESERVATIONS — cap reached). step 2: 8474de0 → 54b807e → 517bf97 (REJECTED iter 1-3) → this commit (iter 4 pending). step 3+: pending. | iter 1 REJECTED, iter 2 REJECTED, iter 3 REJECTED, iter 4 REJECTED, iter 5 REJECTED (5/5 used; v6 follow-up landed without re-review) — see below | step 1: iter 1→5 REJECTED → DONE_WITH_RESERVATIONS. step 2: iter 1→3 REJECTED → iter 4 pending. | 4 — see "s6 plan iter-5 reservations" below |
...
| 3    | 2026-05-14 | 517bf97 | REJECTED | (major) AC7 handoff authorization was mechanically inert — added `s6_handoff_paths` array but the shell recipe in AC7 never passed it to `git log`, so the handoff file wasn't actually audited. | This commit: AC7 shell recipe rewritten to pass `s6_handoff_paths` into BOTH the Phase A1 subject-policy `git log` AND the Phase B STATUS-row audit `git log`. The handoff path is now mechanically enforced (its commits must have `s6:` subject AND a matching s6-impl-<sha7> STATUS row). |
