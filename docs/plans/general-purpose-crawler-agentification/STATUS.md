...
| s6 | `ExternalCrawlRunner` wires `GraphObservationPort` + replan via feedback-aware planner | IMPL_IN_PROGRESS | f986701 | step 1: 36dd567 → e3ea052 → 5d8c6ed → 5a0bdb2 → 944a15f (all REJECTED iter 1-5) → f749385 (DONE_WITH_RESERVATIONS — cap reached). step 2: 8474de0 (REJECTED iter 1) → 54b807e (REJECTED iter 2) → this commit (iter 3 pending). step 3+: pending. | iter 1 REJECTED, iter 2 REJECTED, iter 3 REJECTED, iter 4 REJECTED, iter 5 REJECTED (5/5 used; v6 follow-up landed without re-review) — see below | step 1: iter 1→5 REJECTED → DONE_WITH_RESERVATIONS. step 2: iter 1→2 REJECTED → iter 3 pending. | 4 — see "s6 plan iter-5 reservations" below |
...
| 2    | 2026-05-14 | 54b807e | REJECTED | (blocker) test 19 didn't enforce that the runner imports `derive_planner_observation_feedback` (plan requires it). (blocker) tests 1/1a used `.get(key) is None` which passes when keys are missing; plan requires "keyed always" presence. (major) the s5 boundary test edit was outside the s6 plan's authorized file set. | This commit: runner imports `derive_planner_observation_feedback` (noqa-unused until step 4); test 19 tightened to require the specific symbol; runner.run() now writes the 10 s6 keyed-always fields (all None in legacy / s3 mode) + replan_invoked=False; tests 1/1a now assert key presence + None values; AC7 explicitly authorizes the s5/s6 boundary handoff via new `s6_handoff_paths` array. |
