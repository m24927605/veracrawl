...
| 4    | 2026-05-14 | 5a0bdb2 | REJECTED | (major) plan test 20d required both `clock_trace=None` AND `clock_trace=[]` rejection paths; my test only covered missing-key + empty-list. (minor) STATUS iter-3 row overstated coverage — said tests 20 + 20b "both read `clock.utc_clock_ref`" but test 20 only calls `clock()`. | This commit: extended `test_helper_rejects_missing_or_empty_clock_trace` with the explicit `clock_trace=None` assertion (3 paths now: missing key, None, empty list). STATUS iter-3 wording tightened to name only test 20b as the property-coverage proxy. |
