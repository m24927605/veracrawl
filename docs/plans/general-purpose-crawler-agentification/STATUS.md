...
| s6 | `ExternalCrawlRunner` wires `GraphObservationPort` + replan via feedback-aware planner | IMPL_IN_PROGRESS | f986701 | step 1: 36dd567 → e3ea052 → 5d8c6ed → 5a0bdb2 → 944a15f (all REJECTED iter 1-5) → f749385 (DONE_WITH_RESERVATIONS — cap reached). step 2: 8474de0 → 54b807e → 517bf97 (REJECTED iter 1-3) → 668b30e (APPROVED iter 4). step 3: 1334d9c → f55593a (REJECTED iter 1-2) → this commit (iter 3 pending). step 4: pending. | iter 1 REJECTED, iter 2 REJECTED, iter 3 REJECTED, iter 4 REJECTED, iter 5 REJECTED (5/5 used; v6 follow-up landed without re-review) — see below | step 1: iter 1→5 REJECTED → DONE_WITH_RESERVATIONS. step 2: iter 1→3 REJECTED → iter 4 APPROVED. step 3: iter 1-2 REJECTED → iter 3 pending. | 4 — see "s6 plan iter-5 reservations" below |
...
| 2    | 2026-05-14 | f55593a | REJECTED | (blocker) `clock_trace` producer landed in source but had no matching red test — TDD gap. (major) test 6 call[2] missed the `source_ref == "frontier-admit:discovery"` assertion required by the plan. | This commit: added test 8a `test_s6_mode_populates_clock_trace_in_run_report` — runs the runner in s6 mode and asserts `run_report["clock_trace"]` is a non-empty list with one entry per recorded event, all ISO 8601 UTC datetimes. Test 6 call[2] extended with the missing `source_ref` assertion. AC1 33 → 34. |
