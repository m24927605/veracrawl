...
|-------|-------|--------|-------------|------------------------|-----------------|-----------------|--------------|
| s1 | `CrawlPlannerPort` + plan-decision contract + deterministic fixture adapter | PLAN_DONE_WITH_RESERVATIONS | (uncommitted; lands alongside s1 implementation) | — | iter 1 REJECTED, iter 2 REJECTED, iter 3 REJECTED, iter 4 REJECTED, iter 5 REJECTED (5/5 used; post-iter-5 follow-up landed without re-review) — see below | n/a (no impl yet) | 1 — see "s1 plan iter-5 reservations" below |
