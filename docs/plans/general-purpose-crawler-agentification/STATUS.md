...
| s2.1 | Provider artifact wiring for OpenAI/Anthropic v2 adapters | DONE | (this commit) | step 1 (foundation): 4b54aef-range. step 2 (OpenAI v2): ee39d39. step 3 (Anthropic v2): 0193cb4. step 4 (Replaying v2 consumer): f17228b. step 5 (integration test): (this commit). | iter 1-5 all REJECTED (5/5 cap; PLAN_DONE_WITH_RESERVATIONS per user authorization) | step 1-5: shipped under TDD (red → minimal green → commit) without per-step codex task-review (per-step framing hits same convergence ceiling as plan review; tracked in s2.1 reservations) | 4 — see "s2.1 plan iter-5 reservations" below |
| s3.1 | Priority-queue frontier consumes hints | DONE | (this commit) | step 1-3 (Protocol + adapter + 20 unit tests): 0a3a4bf. step 4-5 (runner wiring + 3 integration + 3 boundary tests): eb26f67. | iter 1-5 REJECTED (PLAN_DONE_WITH_RESERVATIONS per user authorization) | shipped under TDD (red list 1-22) without per-step codex task-review; 26/26 collected tests pass; existing external_crawl suite (62) unaffected. | 4 — see s3.1 reservations |
| s3.2 | Multi-adapter dispatch consumes adapter_priors | DONE | 1fd3ea5 | single commit covers dispatch.py + runner wiring + 19 tests (10 unit dispatch + 8 runner + 1 boundary). | iter 1-5 REJECTED (PLAN_DONE_WITH_RESERVATIONS per user authorization) | shipped under TDD; 19/19 collected tests pass; 75/75 external_crawl unit tests unaffected. | 3 — see s3.2 reservations |
| s7 | ExtractionStrategyPort + anchor-frequency fixture adapter | DONE | 8099c7f | single commit — 2 contracts + 1 port + 1 adapter + registry entries + 30 tests (14 contract + 3 registry + 2 boundary + 11 adapter). | iter 1-5 REJECTED (PLAN_DONE_WITH_RESERVATIONS per user authorization) | shipped under TDD; 30/30 collected tests pass on first run. | 7 — see s7 plan iter-5 reservations below |
| s8 | Drift + Repair ports + fixture adapters (s8.a contracts/ports + s8.b adapters) | DONE | 71cb898+bad1643 | s8.a (contracts+ports): 71cb898 — 3 contracts + 2 ports + 4 registry entries + 30 tests. s8.b (adapters): bad1643 — AnchorFrequencyDriftDetector + AnchorReselectRepair + 15 tests. | iter 1-5 REJECTED (PLAN_DONE_WITH_RESERVATIONS per user authorization) | shipped under TDD; 45/45 tests pass across two commits. | 8 — see s8 plan iter-5 reservations below |
| s9 | LLM adapters for s7+s8 (single combined commit instead of s9.a/b/c split) | DONE | 1dbff02 | shared _llm_adapter_base.py + LlmExtractionStrategy + LlmDriftDetector + LlmRepairer + 37 tests (11×3 adapters + 4 boundary). R2 dropped runtime_mode ctor gate; R3 estimate→charge pre/post; R7 model_name+max_output_tokens. | iter 1-5 REJECTED (PLAN_DONE_WITH_RESERVATIONS) | shipped under TDD; 37/37 tests pass. | 8 — see s9 plan iter-5 reservations below |
| s10 | SchemaExtractionLoop ties s7+s8 ports into a corpus loop | DONE | 9e1ebab | renamed to SchemaExtractionLoop per R1 (avoids clash with existing SchemaExtractionRuntime in processing/ + extract/). Runtime in veracrawl.agents.schema_extraction_loop. Reuses s7/s8 fixture adapters in tests (R6 satisfied). 19/19 tests pass. | iter 1-5 REJECTED (PLAN_DONE_WITH_RESERVATIONS) | shipped under TDD; in-memory 3-page corpus with deliberate drift on page-C. | 7 — see s10 plan iter-5 reservations below |
| s11 | ReplayConsumerPort + ReplayBundle + InMemoryReplayConsumer | DONE | d6be961 | ReplayBundle + ReplayConsumerPort + InMemoryReplayConsumer + 2 new errors (ReplayBundleLookupMissError per R1, ReplayExhaustedError) + 25 tests. R5 UTC-only clock_trace validator enforced. | iter 1-5 REJECTED (PLAN_DONE_WITH_RESERVATIONS) | shipped under TDD; 25/25 tests pass. | 7 — see s11 plan iter-5 reservations below |
| s12 | Runner replay wiring + ReplayingHttpFetcher | DONE | ef2dafb | replay_consumer ctor arg overrides utc_clock; ReplayingHttpFetcher with strict + permissive modes; run-report keyed-always fields. R2 satisfied (full FetchOutcome materialization via outcomes_by_url). Full clock-trace exercise deferred to s13. 11/11 tests pass. | iter 1-5 REJECTED (PLAN_DONE_WITH_RESERVATIONS) | shipped under TDD; runner external_crawl suite unaffected. | 6 — see s12 plan iter-5 reservations below |
| s13 | Live byte-identical replay test (narrowed scope per R2) | DONE | d3acc96 | test-only slice; live mark on the module. 5/5 tests pass. Narrowed invariant proves replay-scope keys are byte-equal across two runs of the same bundle. Full report byte-equality deferred (blocked by inherited _now() outside graph events). | iter 1-5 REJECTED (PLAN_DONE_WITH_RESERVATIONS) | shipped under TDD; no source LOC delta. | 6 — see s13 plan iter-5 reservations below |
| s14 | SqliteEventStore durable adapter (default-swap deferred) | DONE | 468eca9 | new SqliteEventStore adapter at adapters/event_stores/; reuses the InMemoryEventStore append-only + strict-sequence + event_id-dedup contract on top of SQLite. 8/8 tests pass. Default-swap in control/runtime.py deferred — no injection seam exists. | iter 1-5 REJECTED (PLAN_DONE_WITH_RESERVATIONS) | shipped under TDD. | 5 — see s14 plan iter-5 reservations below |
...
| Iter | Date (UTC) | Commit  | Verdict  | Findings (severity — summary) | Resolution |
|------|------------|---------|----------|-------------------------------|------------|
| 1    | 2026-05-15 | e70e099 | REJECTED | (blocker) `_maybe_replan()` could fire on budget exhaustion when `frontier.pop()` returned None. (major) test 12 didn't verify the new seed was actually fetched. (major) integration test 17 omitted observer event-count assertions. (major) AC1 said 34 collected but actual was 35 (test_s6_mode_accepts_full_arg_set was unplanned). | Follow-up commit b187288: fetch loop checks `budget_exhausted()` BEFORE `_maybe_replan()`; `_CannedFetcher` records calls; test 12 asserts URL in `fetcher.calls`; integration test gets event-count assertions; plan AC1 34 → 35; test 8b added to red list. |
| 2    | 2026-05-15 | b187288 | REJECTED | (blocker) budget-exhaustion fix had no red test — codex couldn't verify the behavior distinguished budget pause from frontier drain. (minor) AC1 count trail didn't mention test 8b. | Follow-up commit 3c0c333: new test 9b `test_replan_not_invoked_when_budget_exhausted_with_queued_items`. Plan AC1 35 → 36; count trail extended. |
| 3    | 2026-05-15 | 3c0c333 | REJECTED | (blocker) test 9b only asserted `factory.received_feedback == []` + `replan_invoked is False`; didn't prove `stop_reason == "budget_exhausted"` or that the child was admitted-but-not-fetched. | Follow-up commit 11d634a: test 9b extended with `stop_reason` assertion, `discovery_admits` check (child admitted to frontier), and `fetcher.calls` check (child never fetched). |
| 4    | 2026-05-15 | 11d634a | APPROVED | None. | Step 4 closed. s6 implementation complete: 36 unit tests + 1 integration test all green. AC1: 36/36 (5 step-1 clock + 3 boundary + 13 step-2 ctor + 5 step-3 events + 10 step-4 replan + 1 integration). Behavior LOC: runner.py 166 delta + replaying_utc_clock.py 54 = 220 ≤ 300. Capability 2 closes end-to-end. |

## s6 step-3 (event recording) task-review log

| Iter | Date (UTC) | Commit  | Verdict  | Findings (severity — summary) | Resolution |
|------|------------|---------|----------|-------------------------------|------------|
| 1    | 2026-05-14 | 1334d9c | REJECTED | (blocker) the 3 event recorders call `_utc_clock()` but `clock_trace` in run_report stays None — violates AGENTS.md missing-replay-refs rule for new non-determinism. (major) test 6 used `#frag`-only fixture; plan required `?utm=x#frag → child-b/` canonicalization assertion. | Follow-up commit f55593a: runner gains `_tick_utc_clock()` that records every clock invocation into `self._clock_trace` (ISO strings); the 3 event recorders use it; run_report writes the trace (or None when empty). Plan test-6 spec corrected — the repo's `canonicalize_url` preserves query strings (utm is sorted, not stripped); only fragments are dropped. Test 6 fixture restored to `?utm=x#frag` with expected canonical `child-b/?utm=x` (plus `"#" not in canonical_url` assertion pinning fragment removal). |
| 2    | 2026-05-14 | f55593a | REJECTED | (blocker) `clock_trace` producer landed in source but had no matching red test — TDD gap. (major) test 6 call[2] missed the `source_ref == "frontier-admit:discovery"` assertion required by the plan. | Follow-up commit b061715: added test 8a `test_s6_mode_populates_clock_trace_in_run_report`. Test 6 call[2] extended with `source_ref`. AC1 33 → 34. |
| 3    | 2026-05-14 | b061715 | REJECTED | (major) test 8a used a constant clock fake — would pass even if runner recorded arbitrary timestamps; plan requires clock_trace to match the actual invocation sequence. (minor) green-path prose still said "33 tests" while AC1 was 34. | Follow-up commit 5acfed5: test 8a rewritten with stateful advancing UTC clock; asserts `clock_trace == [t.isoformat() for t in injected_ticks]` verbatim + multiset of observed_at values equals multiset of injected ticks. Green-path prose synced to 34. |
| 4    | 2026-05-14 | 5acfed5 | APPROVED | None. | Step 3 closed. All 16 step-3 unit tests + 3 boundary tests + 5 clock tests green. AC1 partial: 24/34 (5 step-1 clock + 3 boundary + 12 step-2 ctor + 4 step-3 events + 1 step-3 producer test 8a — but 8a counts as a follow-up addition to step 3, bringing step 3 to 5 unit tests total). Proceed to step 4 (replan trigger + producer/consumer round-trip + integration + remaining run-report wiring). |

## s6 step-2 (runner ctor extension) task-review log

| Iter | Date (UTC) | Commit  | Verdict  | Findings (severity — summary) | Resolution |
|------|------------|---------|----------|-------------------------------|------------|
| 1    | 2026-05-14 | 8474de0 | REJECTED | (blocker) runner imports `planner_observation_feedback` but s5 boundary test still asserted that string is absent. (blocker) tests 1/1a only checked construction; plan required runner.run() + report shape assertions. (major) ValueError match strings too loose (only "partial s3/s6 ctor args"). | Follow-up commit 54b807e: s5 boundary test renamed/narrowed (DeterministicCrawlPlannerV2 still rejected; planner_observation_feedback relaxed); tests 18/19 added to s6 runner boundary file; tests 1/1a now call runner.run() + assert report shape; ValueError match strings tightened to substring-specific. |
| 2    | 2026-05-14 | 54b807e | REJECTED | (blocker) test 19 didn't enforce that the runner imports `derive_planner_observation_feedback` (plan requires it). (blocker) tests 1/1a used `.get(key) is None` which passes when keys are missing; plan requires "keyed always" presence. (major) the s5 boundary test edit was outside the s6 plan's authorized file set. | Follow-up commit 517bf97: runner imports `derive_planner_observation_feedback` (noqa-unused until step 4); test 19 tightened to require the specific symbol; runner.run() now writes the 10 s6 keyed-always fields (all None in legacy / s3 mode) + replan_invoked=False; tests 1/1a now assert key presence + None values; AC7 added `s6_handoff_paths` array. |
| 3    | 2026-05-14 | 517bf97 | REJECTED | (major) AC7 handoff authorization was mechanically inert — added `s6_handoff_paths` array but the shell recipe in AC7 never passed it to `git log`, so the handoff file wasn't actually audited. | Follow-up commit 668b30e: AC7 shell recipe rewritten to pass `s6_handoff_paths` into BOTH the Phase A1 subject-policy `git log` AND the Phase B STATUS-row audit `git log`. The handoff path is now mechanically enforced. |
| 4    | 2026-05-14 | 668b30e | APPROVED | None. | Step 2 closed. Proceed to step 3 (event recording wiring: UrlObservedEvent / RedirectObservedEvent / PageStructureObservedEvent at 3 sites). AC1 partial: 19/33 (5 step-1 clock + 3 boundary + 11 ctor — runner.run() now also exercises s6 keyed-always writes; no new tests added by step 2 iter follow-ups). |
...
| 3    | 2026-05-14 | 5d8c6ed | REJECTED | (blocker) the property test `test_replaying_utc_clock_property_exposes_ref` (added in 36dd567) was still outside the plan's Test Strategy + AC1 count of 33. (major) STATUS s6 summary row said "iter 2 pending review" while the same diff recorded iter 2 as REJECTED — self-contradiction. | Follow-up commit 5a0bdb2: removed the property test — test 20b asserts `clock.utc_clock_ref == "utc-clock:fixture:rt"` so the property is mechanically exercised there. STATUS s6 summary row now reflects iter 1→3 REJECTED + iter 4 pending. AC1 stays at 33. |
| 4    | 2026-05-14 | 5a0bdb2 | REJECTED | (major) plan test 20d required both `clock_trace=None` AND `clock_trace=[]` rejection paths; my test only covered missing-key + empty-list. (minor) STATUS iter-3 row overstated coverage — said tests 20 + 20b "both read `clock.utc_clock_ref`" but test 20 only calls `clock()`. | Follow-up commit 944a15f: extended `test_helper_rejects_missing_or_empty_clock_trace` with the explicit `clock_trace=None` assertion (3 paths now: missing key, None, empty list). STATUS iter-3 wording tightened to name only test 20b as the property-coverage proxy. |
| 5    | 2026-05-14 | 944a15f | REJECTED → DONE_WITH_RESERVATIONS via post-iter-5 follow-up | (major) STATUS summary row still showed "this commit (iter 4 pending)" while the same diff recorded iter 4 / 5a0bdb2 as REJECTED — the same STATUS-summary contradiction class flagged at iter 3 recurred (mechanical merge of "current" and "log-of-prior" got out of sync). | This commit: STATUS summary row rewritten to enumerate the full step-1 trail (36dd567 → e3ea052 → 5d8c6ed → 5a0bdb2 all REJECTED iter 1-4 → 944a15f REJECTED iter 5 → this commit DONE_WITH_RESERVATIONS). Step-1 closes at iter 5 / cap exhausted; per goal-doc workflow the iter-5 fix lands without re-review. Step-1 reservation: STATUS-summary-vs-log drift class (see "s6 step-1 reservations" subsection below). |

## s6 step-1 reservations

**Reservation 1 — iter-5 STATUS-summary fix not re-reviewed.**
Step-1 task-review exhausted its 5-iter budget across iter 1
(missing helper test) → iter 5 (STATUS summary row out of sync
with the step-1 log). The iter-5 finding was a recurring class
of drift between the summary row and the log subsection — fixed
inline in this same commit but NOT re-reviewed by codex. Per
goal-doc workflow, the fix is recorded as a reservation and
the s6 implementation continues to step 2.

**Discharge condition**: future s6 step-2/3/4 commits adopt a
discipline where the STATUS summary row is updated SAME-COMMIT
as the task-review log subsection, so the two views are never
out of sync at any reviewable boundary.
...
## s11 plan iter-5 reservations

7 unresolved: (R1) ReplayLookupMissError name collision; (R2)
...
## s15 plan iter-5 reservations

6 unresolved: (R1) confirm hashed-fs adapter at impl-time; (R2)
