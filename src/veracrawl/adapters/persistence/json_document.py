"""Shared JSON-document persistence behavior for concrete adapters."""
...
class JsonDocumentPersistenceAdapterMixin:
    """Port-shaped persistence behavior over adapter-owned JSON document primitives."""
    ...
    def begin_transaction(
        ...
        *,
        ...
        adapter_ref: Ref = "persistence-adapter:json-document",
    ) -> PersistenceTransactionRecord:
    ...
    def list_artifact_refs(self) -> list[Ref]:
        return sorted(
            str(document["id"])
            for document in self._list_documents("artifact_refs")
            if "id" in document
        )
