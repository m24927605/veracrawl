"""Target output type coverage fixture runner CLI."""
...
from veracrawl.contracts.publication import OutputTypeCoverageFixtureManifest
from veracrawl.publish.output_coverage import (
...
def run_output_type_coverage_fixture(
    manifest: OutputTypeCoverageFixtureManifest,
    *,
    profile: str,
) -> OutputTypeCoverageFixtureRunReport:
    result = run_output_type_coverage_gate(
        fixture_id=manifest.id,
        scenario=manifest.scenario,
        policy_decision_refs=[f"policy:{manifest.id}:output-coverage"],
    )
...
def _to_run_report(
    *,
    manifest: OutputTypeCoverageFixtureManifest,
    profile: str,
    result: OutputTypeCoverageRuntimeResult,
) -> OutputTypeCoverageFixtureRunReport:
    report = result.report
    return OutputTypeCoverageFixtureRunReport(
        id=f"output-type-coverage-fixture-run-report:{manifest.id}",
        fixture_id=manifest.id,
        scenario=manifest.scenario,
        profile=profile,
        ...
        covered_output_types=report.covered_output_types,
        missing_output_types=report.missing_output_types,
        unsupported_output_type_refs=report.unsupported_output_type_refs,
        derived_context_as_evidence_refs=report.derived_context_as_evidence_refs,
        missing_type_specific_refs=report.missing_type_specific_refs,
...
def run_fixture(
    ...
    *,
    ...
    manifest = OutputTypeCoverageFixtureManifest.model_validate(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="veracrawl-output-coverage")
