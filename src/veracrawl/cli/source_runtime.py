"""Dynamic source runtime fixture runner CLI."""
...
from veracrawl.fetch.dynamic_source_runtime import (
    DynamicSourceRuntimeGateResult,
    run_dynamic_source_runtime_gate,
)
from veracrawl.runtime_support.logging import bootstrap_cli_logging


class DynamicSourceRuntimeFixtureRunReport(TimestampedModel):
    id: str
    fixture_id: str
    scenario: str
    profile: str
...
def _runtime_records(fixture_id: str) -> list[DynamicSourceRuntimeAdapterRecord]:
    module = _load_module("veracrawl.adapters.sources.dynamic_runtime")
    builder = cast(
        Callable[..., list[DynamicSourceRuntimeAdapterRecord]],
        module.__dict__["build_dynamic_source_runtime_records"],
    )
    return builder(
        fixture_id,
        policy_decision_refs=[f"policy:{fixture_id}:dynamic-source-runtime"],
    )


def run_dynamic_source_runtime_fixture(
    manifest: DynamicSourceRuntimeFixtureManifest,
    *,
    profile: str,
) -> DynamicSourceRuntimeFixtureRunReport:
    records = (
        _runtime_records(manifest.id)
        if manifest.scenario == "dynamic-source-runtime-success"
        else None
    )
    result = run_dynamic_source_runtime_gate(
        ...
        policy_decision_refs=[f"policy:{manifest.id}:dynamic-source-runtime"],
    )
...
def _to_run_report(
    *,
    manifest: DynamicSourceRuntimeFixtureManifest,
    profile: str,
    result: DynamicSourceRuntimeGateResult,
) -> DynamicSourceRuntimeFixtureRunReport:
    report = result.report
    return DynamicSourceRuntimeFixtureRunReport(
        id=f"dynamic-source-runtime-fixture-run-report:{manifest.id}",
        fixture_id=manifest.id,
        scenario=manifest.scenario,
        profile=profile,
        completion_result=report.completion_result,
        operator_status=report.operator_status,
        adapter_record_refs=report.adapter_record_refs,
        ...
        seed_plan_refs=report.seed_plan_refs,
        prior_snapshot_refs=report.prior_snapshot_refs,
        ...
        runtime_adapter_refs=report.runtime_adapter_refs,
...
def run_fixture(
    ...
    *,
    ...
    manifest = DynamicSourceRuntimeFixtureManifest.model_validate(
        _load_json_like(fixture_dir / "manifest.yaml")
    )
    if profile not in manifest.profile_refs:
        raise ValueError(f"fixture {manifest.id} does not support profile {profile}")
    report = run_dynamic_source_runtime_fixture(manifest, profile=profile)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="veracrawl-source-runtime")
...
def main(argv: list[str] | None = None) -> int:
    with bootstrap_cli_logging("veracrawl-source-runtime"):
        parser = build_parser()
